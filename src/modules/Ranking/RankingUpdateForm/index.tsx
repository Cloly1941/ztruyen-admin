...
export type TRankingUpdatePayload = {
    name: string;
    slug: string;
    thumb_url: string;
    authors: string[];
...
const RankingUpdate = ({id, onSuccess}: TRankingUpdate) => {
    ...
    const {data: dataComic, isLoading: isComicLoading, error: errorComic} = useGetMethod<IComic>({
        api: () => ComicService.detail(id),
        key: [CONFIG_QUERY_KEY.COMIC.RANKING_DETAIL, id],
        enabled: !!id && !!open
    });
    ...
    useEffect(() => {
        if (errorComic) {
            toast.error('Không thể tải thông tin truyện. Vui lòng thử lại sau.');
        }
    }, [errorComic]);

    const comic = dataComic?.data

    const {mutate, isPending} = usePatchMethod<TRankingUpdatePayload, IUpdated>({
        api: (payload) => ComicService.update(id, payload),
        keys: [
            [CONFIG_QUERY_KEY.COMIC.RANKING_LIST],
            [CONFIG_QUERY_KEY.COMIC.RANKING_DETAIL, id],
        ],
    });

    const categories = data?.data?.items ?? [];

    const form = useForm<TRankingUpdateForm>({
    ...
    useEffect(() => {
        if (comic) {
            form.reset({
                ...
                chapter_api_data: comic.chapter_api_data,
    ...
    const onSubmit = (values: TRankingUpdateForm) => {
        const payload: TRankingUpdatePayload = {
            ...values,
