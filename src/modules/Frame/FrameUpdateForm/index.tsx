...
const FrameUpdateForm = ({id, onSuccess}: TFrameUpdateForm) => {
    ...
    const {mutate, isPending} = usePatchMethod<TFrameUpdateFormPayload, IUpdated>({
        api: (payload) => FrameService.update(id, payload),
        keys: [
            [CONFIG_QUERY_KEY.FRAME.LIST],
            [CONFIG_QUERY_KEY.FRAME.DETAIL, id],
        ],
    });
    ...
    return (
        <form id="form-update-frame" onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="flex flex-col items-center gap-3">
                <div className="relative">
                    <Avatar className="size-20 ring-2 ring-border">
                        <AvatarImage src={preview ?? frame.image?.url} alt={frame.name}/>
            ...
            <DialogFooter>
                ...
                <Button type="submit" form="form-update-frame" isLoading={isPending}>
                    Lưu khung
                </Button>
            </DialogFooter>
        </form>
